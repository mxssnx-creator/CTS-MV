...
export function AddActiveConnectionDialog({ 
  ...
  const loadConnections = async () => {
    setLoading(true)
    try {
      // Load enabled connections from settings
      ...
      const enabled = connections.filter((c: Connection) => c.is_enabled !== false)
      ...
      try {
        ...
        const activeIds = activeConnections.map(ac => ac.connectionId)
  ...
  const isAlreadyActive = selectedConnection && activeConnections.includes(selectedConnection)
            ...
            <>
              ...
              <Card className="border-blue-200 bg-blue-50/50">
                <CardContent className="pt-4 text-xs">
                  <p className="text-blue-900 font-semibold mb-2">New Connection Behavior:</p>
                  <ul className="text-blue-800 space-y-1">
                    <li>• Connection will be added in <strong>disabled</strong> state</li>
                    <li>• Enable it in the connections panel to start using</li>
                    <li>• Status is independent from Settings connections</li>
                  </ul>
                </CardContent>
              </Card>
            </>
